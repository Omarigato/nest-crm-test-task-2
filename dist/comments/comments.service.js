...
let CommentsService = class CommentsService {
    ...
    async update(id, updateCommentDto, userId) {
        ...
        if (comment.user_id !== userId)
            throw new common_1.ForbiddenException('Вы не можете редактировать чужой комментарий');
        return await this.commentRepository.save({ ...comment, ...updateCommentDto });
    }
    async remove(id, userId) {
        const comment = await this.findOne(id);
        if (comment.user_id !== userId)
            throw new common_1.ForbiddenException('Вы не можете удалить чужой комментарий');
